...
public class AdminManageMenuStaff extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerView = findViewById(R.id.acitivty_admin_manage_menu_staff_recyclerview);
        ...
        firebaseRecyclerOptions = new FirebaseRecyclerOptions.Builder<AdminManageMenuStaffRecyclerClass>().setQuery(databaseReference, AdminManageMenuStaffRecyclerClass.class).build();
        firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<AdminManageMenuStaffRecyclerClass, AdminManageMenuStaffHolder>(firebaseRecyclerOptions) {
            @Override
            protected void onBindViewHolder(@NonNull AdminManageMenuStaffHolder holder, int position, @NonNull final AdminManageMenuStaffRecyclerClass model) {
                ...
                holder.mView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        DatabaseReference ref = FirebaseDatabase.getInstance().getReference()
                                .child("Staff");
                        ref.child(schID).child(FirebaseAuth.getInstance().getCurrentUser().getUid()).addValueEventListener(new ValueEventListener() {
                            @Override
                                    ...
                                    for(DataSnapshot detailSnao: dataSnapshot.getChildren()){
                                        if(detailSnao.getKey().equals("staffID")){
                                            if(detailSnao.getValue().toString().equals(model.getStaffID())){
                                                Toast.makeText(getApplicationContext(), "Own Account", Toast.LENGTH_LONG).show();
                                            } else {

                                                Intent i = new Intent(getApplicationContext(), AdminManageMenuStaffDialog.class);
            ...
            @NonNull
            @Override
            public AdminManageMenuStaffHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    public static class AdminManageMenuStaffHolder extends RecyclerView.ViewHolder {
        ...
        public void setfullname(String fullname) {
            fullnameTextView = mView.findViewById(R.id.activity_admin_manage_menu_staff_recyclerview_item_name);
            fullnameTextView.setText(fullname);
        }

        public void setstatus(String status) {
            statusTextView = mView.findViewById(R.id.activity_admin_manage_menu_staff_recyclerview_item_usertype);
            statusTextView.setText(status);
        }

        public void setstaffID(String staffID) {
            staffIDTextView = mView.findViewById(R.id.activity_admin_manage_menu_staff_recyclerview_item_staffid);
