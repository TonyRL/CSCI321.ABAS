...
public class FileSharingAdd extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    firebaseOptions = new FirebaseRecyclerOptions.Builder<FileSharingAddUserRetrieveClass>()
        .setQuery(allDatabaseUserReference, FileSharingAddUserRetrieveClass.class).build();

    firebaseRecyclerAdapter =
        new FirebaseRecyclerAdapter<FileSharingAddUserRetrieveClass, FileSharingAddViewHolder>(
            firebaseOptions) {
          @Override
          protected void onBindViewHolder(@NonNull final FileSharingAddViewHolder holder,
              final int position, @NonNull FileSharingAddUserRetrieveClass model) {
          ...
          @NonNull
          @Override
          public FileSharingAddViewHolder onCreateViewHolder(@NonNull ViewGroup parent,
              int viewType) {
            View view1 = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.activity_file_sharing_add_user_recyclerview_layout, parent,
                    false);
              ...
              public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                if (task.isSuccessful()) {
                  ...
                  DatabaseReference passDB1 = FirebaseDatabase.getInstance().getReference()
                      .child("Shared_Files").child(currentUserID).child(messsage_push_id)
                      .child(idToPass);

                  DatabaseReference passDB2 = FirebaseDatabase.getInstance().getReference()
                      .child("Shared_Files").child(idToPass).child(messsage_push_id)
                      .child(currentUserID);
