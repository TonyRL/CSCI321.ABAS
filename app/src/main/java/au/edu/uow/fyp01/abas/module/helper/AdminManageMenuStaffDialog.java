...
public class AdminManageMenuStaffDialog extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    ArrayAdapter<CharSequence> dataAdapter = ArrayAdapter
        .createFromResource(getApplicationContext(), R.array.user_type_arrays,
    ...
    button.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View view) {
        if (finalUserType.equals("")) {
          Toast.makeText(getApplicationContext(), "No selected choice made", Toast.LENGTH_LONG)
              .show();
        }
                      ...
                      FirebaseDatabase.getInstance().getReference().child("User").child(UID)
                          .updateChildren(map, new DatabaseReference.CompletionListener() {
                            ...
                            public void onComplete(DatabaseError databaseError,
                              ...
                              Toast.makeText(getApplicationContext(), "Completed user node",
                ...
                for (DataSnapshot detailsSnap : uidSnap.getChildren()) {
                  if (detailsSnap.getKey().equals("staffID")) {
                    if (detailsSnap.getValue().toString().equals(staffID)) {
                      Map map = new HashMap();
                      map.put("usertype", finalUserType);
                      FirebaseDatabase.getInstance().getReference().child("Staff").child(schID)
                          .child(UID)
                          .updateChildren(map, new DatabaseReference.CompletionListener() {
                            @Override
                            public void onComplete(DatabaseError databaseError,
                                DatabaseReference databaseReference) {
                              Toast.makeText(getApplicationContext(), "Completed staff node",
                                  Toast.LENGTH_LONG).show();
